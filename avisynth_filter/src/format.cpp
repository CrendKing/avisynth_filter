...
const std::vector<Format::FormatInfo> Format::FORMATS = {
...
auto Format::MediaTypeInfo::operator!=(const MediaTypeInfo &other) const -> bool {
    return formatIndex != other.formatIndex
...
auto Format::LookupMediaSubtype(const CLSID &mediaSubtype) -> int {
    for (int i = 0; i < static_cast<int>(FORMATS.size()); ++i) {
        if (mediaSubtype == FORMATS[i].mediaSubtype) {
            return i;
        }
    }
...
auto Format::GetMediaTypeInfo(const AM_MEDIA_TYPE &mediaType) -> MediaTypeInfo {
    MediaTypeInfo info;
    ...
    info.formatIndex = LookupMediaSubtype(mediaType.subtype);
    ...
    info.videoInfo.pixel_type = FORMATS[info.formatIndex].avsType;
    info.videoInfo.num_frames = NUM_FRAMES_FOR_INFINITE_STREAM;

    return info;
}

auto Format::CopyFromInput(int formatIndex, const BYTE *srcBuffer, int srcPixelStride, BYTE *dstSlices[], const int dstStrides[], int rowSize, int height, IScriptEnvironment *avsEnv) -> void {
    const FormatInfo &format = FORMATS[formatIndex];

    const int absHeight = abs(height);
    const int srcStride = srcPixelStride * format.bytesPerComponent * format.componentsPerPixel;
    const int srcDefaultPlaneSize = srcStride * absHeight;

    const BYTE *srcDefaultPlane;
    int srcDefaultPlaneStride;
    if ((format.avsType & VideoInfo::CS_BGR) != 0 && height < 0) {
    ...
    if ((format.avsType & VideoInfo::CS_INTERLEAVED) == 0) {
        // 4:2:0 unpacked formats

        if (format.mediaSubtype == MEDIASUBTYPE_IYUV || format.mediaSubtype == MEDIASUBTYPE_I420 || format.mediaSubtype == MEDIASUBTYPE_YV12) {
            ...
            const BYTE *srcU;
            const BYTE *srcV;
            if (format.mediaSubtype == MEDIASUBTYPE_YV12) {
...
auto Format::CopyToOutput(int formatIndex, const BYTE *srcSlices[], const int srcStrides[], BYTE *dstBuffer, int dstPixelStride, int rowSize, int height, IScriptEnvironment *avsEnv) -> void {
    const FormatInfo &format = FORMATS[formatIndex];

    const int absHeight = abs(height);
    const int dstStride = dstPixelStride * format.bytesPerComponent * format.componentsPerPixel;
    ...
    if ((format.avsType & VideoInfo::CS_INTERLEAVED) == 0) {
        if (format.mediaSubtype == MEDIASUBTYPE_IYUV || format.mediaSubtype == MEDIASUBTYPE_I420 || format.mediaSubtype == MEDIASUBTYPE_YV12) {
            ...
            Interleave(srcSlices[1], srcSlices[2], srcStrides[1],
                       dstUVStart, dstStride, rowSize / 2, absHeight / 2, format.bytesPerComponent);
        }
    }
}
